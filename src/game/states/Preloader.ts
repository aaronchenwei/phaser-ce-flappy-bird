...
export default class Preloader extends Phaser.State {
  ...
  public loadResources() {
    const image = this.load.image.bind(this.load);
    const spritesheet = this.load.spritesheet.bind(this.load);
    const audio = this.load.audio.bind(this.load);
    ...
    // Text
    ['title', 'ready', 'tap', 'over'].forEach(el =>
      image(el, `assets/text/${el}.png`)
    );
    ...
    // Audio
    ['die', 'hit', 'point', 'swooshing', 'wing'].forEach(el =>
      audio(el, `assets/audio/${el}.wav`)
    );

    // Medals
    ['bronze', 'gold', 'platinum', 'silver'].forEach(el =>
      image(el, `assets/medals/${el}.png`)
    );
  }
